...
## Medidas de tendencia central: ¿Qué valor "representa" al grupo?
...
set.seed(123)
datoss <- rnorm(1000, mean = 10, sd = 2)

# Calcular la media y la mediana
media <- mean(datoss)
mediana <- median(datoss)

# Crear el gráfico
ggplot(data = data.frame(x = datoss), aes(x = x)) +
  geom_histogram(binwidth = 0.5, aes(y = ..density..), fill = "lightblue", color = "black") +
...
4. Dividimos entre el número de ingresos menos 1 (n-1): 1000 / 4 = **250**.]

.pull-rigth[

<img src="img/var.png" width="380px"/> 

- Interpretación: la varianza de los ingresos es 250 mil pesos **al cuadrado**, entre esas 5 personas (para sacar las diferencias respecto a la media tuvimos que elevar al cuadrado) 
]
...
### Continuando con ejemplo anterior: 
  + Varianza de los ingresos fue de 250 miles de dólares **al cuadrado**. 
  + Para obtener la desviación estándar, solo necesitamos calcular la raíz cuadrada de la varianza: 
  + sqrt(250) = 15.811.
  + o con función sd()
  
-  Interpretación 
  + En promedio los ingresos se alejan en 15.811 de la media. 
...
- la cada: representa el rango intercuartílico (IQR). Va desdeel primer cuartil (25%) y el tercer cuartil (75%), abarcando un 50% de los datos. 
