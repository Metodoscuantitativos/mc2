...
#   La estadística descriptiva se centra en la organización, resumen y presentación de datos para describir características de una muestra o población, como promedios, desviaciones estándar y gráficos.

#La estadística inferencial, en cambio, utiliza muestras de datos para hacer inferencias o generalizaciones 
#sobre una población más amplia, mediante técnicas como pruebas de hipótesis, intervalos de confianza y análisis de regresión.

### Esta respuesta es muy parecida a la de otra de sus compañeras. ojo con los plaguios
...
#   significa que aunque dos variables puedan estar correlacionadas (es decir, que sus valores tienden a cambiar juntos de manera predecible), esto no implica necesariamente que una variable cause directamente cambios en la otra. 
#La correlación solo indica la relación estadística entre las variables, pero no proporciona información sobre la dirección o la causalidad de esa relación.

### En general está bien
### La analicé con una app y sale que se utilizó IA 

# 7. ¿cuáles son las diferencias entre correlación y covarianza? y ¿cuáles son sus similitudes?
#Correlación: Es una medida estandarizada que indica la fuerza y dirección de la relación lineal entre dos variables. Está normalizada para estar entre -1 y 1.
#Covarianza: Es una medida bruta que indica la dirección de la relación entre dos variables. Puede tomar cualquier valor real y no está normalizada.
#Ambas son medidas que indican la relación entre dos variables.
#Ambas son útiles para entender cómo las variables se relacionan entre sí en un conjunto de datos.

### 65% de uso de IA en esta respuesta
...
names(base)
base <- base_antropologia_limpia %>% dplyr::rename(rend_academ=ea_05_satisfaccion_rendimiento_academico,
                                                         carga_academ= ea_03_descripcion_carga_academica) #Aquí hay una recodificación que no se solicita en este Item.
...
base <- base_antropologia_limpia %>% dplyr::rename(rrss_fav=rs_05,
                                                   tiempo_libre= to_01,
                                                   expec_soc=rs_03,
                                                   estres_RRSS=rs_04)

### Error: object 'base_antropologia_limpia' not found
### Se aplicó mal el código
### Corrección: 
base <- base %>% dplyr::rename(rrss_fav=rs_05,
tiempo_libre= to_01,
expec_soc=rs_03,
estres_RRSS=rs_04)
...
base <- base_antropologia_limpia %>% rename(lugar_musica=cm_05)
...
### NULL 
### No se puede obtener ni aplicar esta función sobre datos que no son válidos
### Esto ocurre porque la recodificación no se hizo
...
### Error in strsplit(base$lugar_musica, ",") : non-character argument
### El código utilizado es el correcto, pero no se logra aplicar porque arriba no se hizo correctamente la recodificación

# 03.4 Transforme el objeto respuestas a un vector con la función unlist (0.5 pto)

respuestas <- unlist (respuestas) 

### Error: object 'respuestas' not found
### El código utilizado es el correcto, pero no se logra aplicar porque arriba no se hizo correctamente la recodificación
...
CONSUELO_LLANTEN<-  freq(respuestas, prop=TRUE, order = "freq", report.nas = FALSE) %>% 
  tb()

### No se logra realizar el ejercicio, porque la recodificación no se realizó correctamente
...
base <- base_antropologia_limpia %>% rename(religion_rec=re_02)
...
# Utilice la siguiente estructura de código 
                                               ...
                                               base <- base %>%
                                                 mutate(religion_rec = case_when(
                                                   religion_rec %in% c("No tengo una afiliación religiosa, pero si me considero una persona espiritual (por ejemplo, creo en las energías)",
                                                                       "seria en creer en alguien superior un tipo de energía que se interpreta de diversas formas en la religión pero no es alguien al cual se le pueda poner un nombre",
                                                                       "deísmo") ~ "_ _ _ _",
...
#                    Poco tiempo.
base <- base_antropologia_limpia %>% rename(tiempo_libre=to_01)

base <- base%>% mutate(to_01=case_when(tiempo_libre %in% c("Bastante tiempo.", "Suficiente tiempo.")~"tiempo disponible",
                                        tiempo_libre %in% c("No tengo tiempo.", "Poco tiempo.")~ "tiempo limitado",
                                        TRUE~tiempo_libre))
names(base)
table(base$tiempo_libre)
...
ggplot(CONSUELO_LLANTEN, aes(x = pct, y = fct_reorder(respuestas, pct), fill= respuestas)) +
  geom_col() +
