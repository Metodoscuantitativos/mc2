...
# por una parte la hipótesis nula (H₀) representa una posición de no cambio o no diferencia, mientras que la hipótesis alternativa (H₁) representa la presencia de un cambio o diferencia.

### La respuesta es imprecisa pero no está del todo mal
...
#   la estadística descriptiva se ocupa de describir y resumir los datos actuales, y la estadística inferencial por otra parte se centra en hacer predicciones y generalizaciones sobre una población a partir de una muestra.

### Bien

# 5. ¿cuál es la diferencia entre las pruebas de contraste de hipótesis de Chi cuadrado con ANOVA?
#   la prueba del Chi cuadrado se utiliza para analizar relaciones entre variables categóricas, mientras que ANOVA se emplea para comparar medias de una variable continua entre varios grupos categóricos.
...
# base <- base %>% dplyr::rename(Nombre nuevo = nombre antiguo,  
#                                ______ = ______,
#                                ______ = ______, 
#                                ______ = ______)
