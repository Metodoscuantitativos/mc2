...
#
# La hipótesis nula establece que (1)no hay efecto entre las variables de interés y (2) que cualquier observación se debe
# únicamente a la variabilidad aleatoria del proceso o la muestra. Por otro lado, la hipótesis alternativa sugiere que hay 
# una diferencia o efecto significativo entre las variables. De esta forma, ña hipótesis alternativa muestra el interés del investigador
# en demostrar que los datos presentan evidencia suficiente para rechazar la hipótesis nula a favor de la alternativa. 
# Cabe decir que la elección entre estas dos hipótesis depende de la pregunta de investigación y de la dirección de la prueba 
# estadística que se esté realizando. 

### En general está bien, pero hay ciertos detalles que no son del todo correctos. 
...
#
# En primer lugar, la estadística descriptiva se enfoca en describir y resumir datos utilizando medidas como la media y la desviación
# estándar, también gráficos y diagramas de barras. El objetivo de la estadística descriptiva es llegar a una comprensión básica de los
# datos. Por su parte, la estadística inferencial, como dice su nombre, busca hacer inferencias sobre una población más amplia a partir
# de datos muestrales, utilizando técnicas como la teoría de probabilidad y pruebas de hipótesis. El fin es generalizar resultados y poder 
# tomar decisiones más allá de los datos observados. 

### Bien :)
#   
# 7. ¿cuáles son las diferencias entre correlación y covarianza? y ¿cuáles son sus similitudes?

# Tanto la correlación como la covarianza son medidas estadísticas que indican cómo dos variables están relacionadas,
# pero se diferencian en su escala y capacidad de interpretación. La covarianza cuantifica la dirección de la relación
# lineal entre dos variables, pero no estandariza la magnitud, por lo que sus unidades dependen de las variables en cuestión,
# dificultando la comparación entre diferentes pares de variables. 
#Por otro lado, la correlación estandariza la covarianza dividiéndola por el producto de las desviaciones estándar de las 
# variables, produciendo un valor entre -1 y 1, lo que sí facilita la comparación e interpretación de la relación. 
# Ambas medidas son similares en que indican la relación y dirección entre dos variables, pero la correlación es más útil
# para comparar relaciones en distintos contextos debido a su naturaleza normalizada.
#   
...
nrow(base)
ncol(base)
#Gracias a los códigos, se puede saber que los datos tienen 147 filas (nrow) y 60 columnas (ncol)
...
library(dplyr)
 base <- base %>%dplyr::rename(nivel_informacion = ma_01, interes_participacion = ma_02, participacion_grupos = ma_03, participacion_eventos = ma_04, adquisicion_productos = ma_05)
...
julia_sotomayor <-  freq(respuestas, prop=TRUE, order = "freq", report.nas = FALSE) %>% 
  tb()
...
desviacion_estandar <- sd(base$ea_04_notas_ultimo_semestre, na.rm = TRUE)
...
ggplot(julia_sotomayor, aes(x = pct, y = fct_reorder(respuestas, pct), fill=respuestas)) +
  geom_col() +
