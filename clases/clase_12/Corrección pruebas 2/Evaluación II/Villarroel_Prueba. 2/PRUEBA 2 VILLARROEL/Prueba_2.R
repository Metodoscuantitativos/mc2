...
#   R:La estadística inferencial se enfoca en hacer inferencias o predicciones sobre una población basándose en una muestra representativa de datos. Algunos de los métodos empleados son las pruebas de hipótesis, intervalos de confianza, etc.
#   La estadística inferencial permite estimar parámetros poblacionales y probar hipótesis, extrapolando los resultados de la muestra a la población general. Ayudando a tomar decisiones y hacer predicciones fundamentadas sobre una población más amplia, más allá de los datos observados directamente.

### La respuesta está bien por un lado, pero sólo se habla de la estadística inferencial, y no de la estadística descriptiva.

# 5. ¿cuál es la diferencia entre las pruebas de contraste de hipótesis de Chi cuadrado con ANOVA?
#   R: En una prueba de contraste de hipotesis Chi cuadrado se utiliza cuando contamos con dos variables categóricas (nominales u ordinales) y Anova se utiliza cuando contamos con una variable cuantitativa y otra categórica (variable de agrupación).
#   
### Bien

# 6. ¿a qué se refiere con que la correlación no establece una dependencia entre variables?
#   R: Cuando se dice que la correlación no establece una dependencia entre variables, se refiere a que la correlación solo mide la fuerza y la dirección de una relación lineal entre dos variables, pero no implica que una variable cause cambios en la otra.
...
base <- base %>% dplyr::rename(religion_rec = re_o2) 
names(base)
