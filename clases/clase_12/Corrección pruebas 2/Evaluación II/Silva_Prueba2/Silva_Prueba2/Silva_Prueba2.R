...
# Los estadísticos de tendencia central son medidas que describen el punto medio o valor típico de un conjunto de datos. Nos ayudan a entender dónde se concentran los valores en una distribución.
#Dos ejemplos de estadísticos de tendencia central son la media aritmética y la mediana. La media aritmética es el promedio de todos los valores. Por ejemplo, si tenemos las edades 20, 25, 30, 35 y 40 años, la media sería (20+25+30+35+40) / 5 = 30 años.
#La mediana, por otro lado, es el valor que está justo en el medio cuando ordenamos los datos. En el mismo ejemplo, al ordenar las edades (20, 25, 30, 35, 40), la mediana sería 30 años.
#La principal diferencia entre las medidas de tendencia central y las medidas de dispersión es que las primeras nos indican dónde se concentran los datos, mientras que las segundas nos muestran qué tan esparcidos están los datos alrededor de ese centro.
#Un ejemplo práctico en antropología podría ser el estudio de la estatura de los miembros de una tribu. La media nos diría la estatura típica (por ejemplo, 165 cm), mientras que una medida de dispersión como la desviación estándar nos indicaría qué tan variables son las estaturas (por ejemplo, ±10 cm). 
#Esto nos ayudaría a entender no solo la estatura promedio, sino también si hay mucha o poca variación en las estaturas de los miembros de la tribu.

### Bien. La respuesta es muy clara y precisa.
### Se detecta uso de IA

# 4. Cuál es la diferencia entre la estadística descriptiva y la estadística inferencial?
# La estadística descriptiva se limita a la recopilación, organización, resumen y presentación de datos de modo que sea posible comprender la estructura de esta información y sus características fundamentales. 
#En general se trata de medidas como la media, la mediana, la moda, y de gráficos de barra, histogramas y de dispersión. 
#En cambio, la estadística inferencial se enfoca en utilizar una muestra de datos para generar generalizaciones, estimaciones o predicciones acerca de un conjunto más amplio, mediante métodos estadísticos como las pruebas de hipótesis, 
#los intervalos de confianza y las regresiones. 
#Es decir, la estadística descriptiva describe lo ocurrido en el ámbito del estudio específico, mientras que la inferencial busca relacionar los datos presentes con la población en general.

### Bien. La respuesta es muy clara y precisa.
### Se detecta uso de IA
  
# 6. ¿a qué se refiere con que la correlación no establece una dependencia entre variables?
#Al tener una correlación alta entre dos variables, se entiende un alto grado y una dirección en la que varían juntas, lo que no afecta la relación causal o la dependencia mutua entre ellas. 
#Si dos variables presentan una alta correlación (ya sea positiva o negativamente), no implica que una causa la otra no. 
#La correlación no establece una causa y efecto, ya que no considera que puede haber variables confusas y factores externos que actúan en ambas variables. Por lo tanto, no implica que una sea la causa de cómo varía la otra. 
#Por lo tanto, la correlación suele ser causada por la coincidencia o la causalidad de las terceras variables y no por la conexión directa en la cual una variable causa el cambio en la otra.
...
base <- base %>% dplyr::rename(Nivel_Información = ma_01,  
                               Interés_actividades = ma_02,
                               Interés_proyectos = ma_03, 
                               Participación_actividades = ma_04)
...
alonso_silva <- freq(respuestas, prop=TRUE, order = "freq", report.nas = FALSE) %>% 
  tb()
...
desviación_estandar<- sd(base$ea_04_notas_ultimo_semestre, na.rm = TRUE)
# varianza (0.5 ptos)
...
ggplot(alonso_silva, aes(x = pct, y = fct_reorder(respuestas, pct), fill=respuestas)) +
  geom_col() +
