...
# R: La  hipótesis nula es la idea inicial de la investigación, mientras   que la hipótesis alternativa es aquella que es contraria a 
#la nula, permite abarcar más resultados considerando la población y parámetros estudiados, ambas son necesarias en el contraste de hipótesis
#ya que en esta etapa se reunen y analizan los datos para saber cuál es la más acertada.
...
# 2. ¿cuál es la manera en que se debe interpretar el valor-p y cómo repercute este valor sobre la decisión acerca del contraste de hipótesis?
  ...
  #El valor-p ayuda a determinar si los resultados observados son lo suficientemente 
#extremos como para rechazar o aceptar la hipótesis nula. 

### En general está bien, aunque la respuesta es poco precisa y no da a entender cómo se debe interpretar este valor
...
#Las pruebas de contrast de hipótesis Chi evalua la independencia de dos variables categóricas, comparando desde la 
#frecuencias observadas con las esperadas bajo la hipotesis nula. Mientras que con Anova se compara las medias,
# de tres o más grupos para identificar diferencias significativas.

### En general está bien
#   
# 6. ¿a qué se refiere con que la correlación no establece una dependencia entre variables?
#La correlacional no establece dependientes pues busca ver relaciones entre variables.
#   
...
View("base_antropologia_limpia.xlsx") 
...
#cantidad de casos: 147

ncol(base) 
 #cantidad de variables: 60
...
ggplot(Mariana_Pérez, aes(x = pct, y = fct_reorder(respuestas, pct), fill=respuestas)) +
  geom_col() +
