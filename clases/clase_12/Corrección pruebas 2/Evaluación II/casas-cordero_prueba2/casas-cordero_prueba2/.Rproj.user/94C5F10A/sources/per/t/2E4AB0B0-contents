...
#Respondí la 1, la 2 y la 4.

# 1. Describa la diferencia entre la hipótesis nula y la hipótesis alternativa en un contraste de hipótesis.
# La hipótesis nula es un osición de base que indica que no hay significancia o efecto entre las variables.
#Esta es la hipótesis que se busca rechazar. Para esto se declara una segunda hipótesis, la alternarttiva, en donde se busca
#evidencia o razones suficientes para demostrar que sí hay una relación significativa entre las variables. 
...
# Las medidas de tendencia central son un conjunto de herramientas estadísticas que se usan para encontrar un valor representativo de un conjunto de datos. Dos ejemlos de estos datos }
#son la media y la mediana. La media por su lado es el promedio del conjunto de datos. Y por el otro lado, la mediana es el valor central del conjunto de datos ordenados.

### Súper!

# 4. Cuál es la diferencia entre la estadística descriptiva y la estadística inferencial?
#La estadística descriptiva es la que describe y resume variables que refieren directamente al universo universo, y la estadística inferencial examina variables que refieren a 
#una muestra del universo. Y realiza inferencias a través de pruebas de hipótesis de significación estadística, como la encuesta que se hizo en el curso.

### Bien. Igualmente es importante mencionar que la estadística inferencial permite generalizar, pues permite extrapolar los resultados desde una muestra a la población,
### mientras que la estadística descriptiva sólo describe y caracteriza la muestra seleccionada utilizando medidas de tendencia central y tablas básicas.
...
# 01. Cargar datos-------------------------------------------------------- 
base <- read.xlsx("datos/base_antropologia_limpia (4).xlsx")
...
base <- base %>% dplyr::rename (horas_estudio_semana1 = ea_01_horas_estudio_semana,
                                descripcion_carga_academica = ea_03_descripcion_carga_academica,
                                notas_ultimo_semestre = ea_04_notas_ultimo_semestre,
                                nivel_estres_ultimo_semestre = ea_06_nivel_estres_ultimo_semestre)
...
# el nombre del objeto debe ser su "nombre_apellido", (0.5 pto)
 ...
 noel_casascordero <-  freq(respuestas, prop=TRUE, order = "freq", report.nas = FALSE) %>% 
  tb()
...
# 04.2 recodifique las categorías de esta variable en una nueva variable denominada 
#"religion_rec" de la siguiente forma (3 ptos)
...
unique(base$notas_ultimo_semestre) #en mi caso como lo recodifiqué, se llama notas_ultimo_semestre
...
# Mediana (0.5 ptos)
median_notas <- median(base$notas_ultimo_semestre, na.rm = TRUE)
median_notas #5.8

# desviación estándar (0.5 ptos)
sd_notas <- sd(base$notas_ultimo_semestre, na.rm = TRUE)
sd_notas #0.4721033

# varianza (0.5 ptos)
var <- var(base$notas_ultimo_semestre, na.rm = TRUE)
var #0.2228816
...
ggplot(base, aes(x=religion_rec, y=notas_ultimo_semestre, fill=religion_rec)) + 
  ...
  labs(title = "Promedio de notas según tipo de religión", 
       ...
       y= "Notas",
       x= "Tipo de religión",
       fill = "Tipo de religión")+
  theme(legend.position="none")


# 07.2 ea_04_notas_ultimo_semestre vs to_01 (3 ptos)
unique(base$notas_ultimo_semestre)
unique(base$to_01)
...
ggplot(noel_casascordero, aes(x = pct, y = fct_reorder(respuestas, pct), fill=respuestas)) +
  geom_col() +
