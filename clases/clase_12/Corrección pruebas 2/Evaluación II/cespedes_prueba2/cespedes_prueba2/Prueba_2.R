...
# En un contraste de hipótesis, se utilizan la hipótesis nula y la hipótesis alternativa para evaluar suposiciones sobre una población basándose en datos.
# La hipótesis nula es una afirmación que se considera verdadera hasta que se demuestre lo contrario. Propone que no hay una relación significativa entre las variables estudiadas o que un parámetro de la población es igual a un valor específico. 
# Su propósito es actuar como un punto de referencia inicial, que buscamos rechazar con evidencia en contra. Por ejemplo, podríamos decir que la media de las notas de los estudiantes no ha cambiado.
# En contraste, la hipótesis alternativa propone que sí hay una diferencia o relación significativa. Es la afirmación que queremos probar y aceptar si encontramos suficiente evidencia en los datos. 
# Por ejemplo, podríamos decir que la media de las notas ha cambiado.
# La diferencia clave entre ambas es su rol en la prueba: la hipotesis nula es el estado que se asume cierto hasta que se demuestre lo contrario, mientras que hipotesis alternativa es lo que se aceptará si H0H_0H0 es rechazada. Así, H0H_0H0 y HAH_AHA son mutuamente excluyentes; solo una puede ser verdadera.

### Súper bien! :) Excelente respuesta
...
# La diferencia entre la estadística descriptiva y la estadística inferencial radica en lo que queremos hacer con los datos que tenemos.
# La estadística descriptiva es como tomar una foto de nuestros datos. Nos ayuda a resumir y entender la información que hemos recopilado. Por ejemplo, si estamos investigando las notas de los estudiantes en un curso de antropología, 
# la estadística descriptiva nos diría cuál es la nota promedio, la más alta y la más baja, y cómo se distribuyen las notas entre todos los estudiantes. 
# Utiliza herramientas como medias, medianas, gráficos y tablas para darnos una imagen clara y comprensible de los datos actuales. En otras palabras, nos dice "qué hay" en los datos que hemos observado.

# La estadística inferencial va un paso más allá. Imagina que solo tenemos las notas de una pequeña muestra de estudiantes, pero queremos saber cómo le está yendo a toda la facultad de antropología. Aquí es donde entra la estadística inferencial. 
# Nos permite hacer predicciones o generalizaciones sobre una población más grande a partir de esa pequeña muestra. Utiliza métodos como pruebas de hipótesis y intervalos de confianza para estimar parámetros y sacar conclusiones más amplias. 
# Es como usar nuestra muestra para hacer una afirmación educada sobre un grupo mucho más grande. Por ejemplo, podríamos usar las notas de nuestra muestra para estimar cuál es la nota promedio de todos los estudiantes de la facultad.
# En resumen, la estadística descriptiva nos ayuda a entender y resumir los datos que tenemos, mientras que la estadística inferencial nos permite hacer predicciones y generalizaciones sobre una población más amplia basándonos en esos datos.
# Ambas son herramientas esenciales en la investigación y nos ayudan a transformar datos en conocimiento útil.
#   
### Excelente! :)
#   
...
#El proceso de inferencia se dice que sigue una lógica inductiva porque parte de observaciones específicas para llegar a conclusiones generales. En estadística inferencial, utilizamos datos de una muestra pequeña y limitada para hacer generalizaciones sobre una población más amplia. 
#Esto es fundamental porque no siempre es posible o práctico estudiar a toda la población. Por ejemplo, si estamos investigando hábitos de estudio en estudiantes de antropología, podríamos encuestar a un grupo pequeño de estudiantes sobre sus horas de estudio diarias. 
#A partir de estos datos específicos, la estadística inferencial nos permite hacer estimaciones sobre todos los estudiantes de antropología de la universidad. Utilizando técnicas como intervalos de confianza o pruebas de hipótesis, 
#analizamos los datos de la muestra y extrapolamos los resultados. Aunque solo hayamos encuestado a un pequeño grupo, podemos inferir que el promedio de horas de estudio en la población total de estudiantes de antropología es similar.

#La lógica inductiva es crucial porque nos permite hacer afirmaciones útiles y significativas sobre grandes poblaciones a partir de muestras más manejables. Aunque siempre existe un grado de incertidumbre, las herramientas estadísticas nos ayudan a cuantificar esta incertidumbre y a hacer inferencias más precisas. 
#En resumen, el proceso de inferencia sigue una lógica inductiva porque toma datos específicos de una muestra y los utiliza para hacer generalizaciones sobre una población más grande, permitiendo transformar observaciones limitadas en conclusiones amplias y significativas.
...
nrow(base) 
ncol(base) 
### Faltó indicar el número de filas/casos y columnas/variables
...
base <- base %>% dplyr::rename(Nivel_informacion_basura = ma_01,  
                               Interes_actividades_educativas = ma_02,
                                Participacion_proyectos_ecologicos = ma_03, 
                                Participacion_actividades_ecologicas = ma_04)
...
matilde_cespedes <-  freq(respuestas, prop=TRUE, order = "freq", report.nas = FALSE) %>% 
  tb()
...
mediana_notas <- median(base$ea_04_notas_ultimo_semestre, na.rm = TRUE)
...
ggplot(matilde_cespedes, aes(x = pct, y = fct_reorder(respuestas, pct), fill=respuestas)) +
  geom_col() +
