...
# La hipotesis nula es una afirmación que se cree verdadera hasta que se demuestre lo contrario con una evidencia suficiente, por el contrario de la hipotesis alternativa que se entiende como una afirmación que es aceptada si los datos proporcionan suficiente evidenciar para rechazar la hiptesis nula.

### En general está bien, pero falta precisión y claridad en la respuesta
...
#las diferencias se encuentran en que la estadistica descriptiva tal y como dice su nombre se enfoca en describir y resumir datos, no alaniza más allá, y utiliza medidas y gráficos para presentar datos, mientras que la estadística inferencial, se enfoca en realizar inferencias y generalizaciones sobre una población a partir de una muestra, utilizando métodos para estimar parámetros y probar hipotesis, buscando sacar conclusiones y predicciones segun estos datos.

### Bien

# 5. ¿cuál es la diferencia entre las pruebas de contraste de hipótesis de Chi cuadrado con ANOVA?
#Chi al cuadrado se usa para el análisis de asociaciones entre variables categóricas, trabajando con tablas de contigencias, mientras ANOVA se usa para el análisis de diferencias entre las medias de grupos y trabaja con datos continuos.  

### En general está bien, sin embargo, ANOVA trabaja con una variable continua, pero además con variables categóricas
...
base <- read.xlsx("C:/Users/Alumno/Desktop/Prueba R_2 Asencio.K/base_antropologia_limpia.xlsx")

### Con ese código, la base sólo puede abrirla en su computador
### Error in read.xlsx.default("C:/Users/Alumno/Desktop/Prueba R_2 Asencio.K/base_antropologia_limpia.xlsx") : 
###   File does not exist.
### El código más acertado hubiera sido:
...
base <- base %>% dplyr:: rename ("Horas_RRSS" = rs_01,  
                                "Razones_RRSS" = rs_02,
                                "Presencia_RRSS" = rs_03, 
                                "Gestion_Estres_RRSS" = rs_04)
...
krishna_asencio <-  freq(respuestas, prop=TRUE, order = "freq", report.nas = FALSE) %>% 
  tb()
...
ggplot(krishna_asencio, aes(x = pct, y = fct_reorder(respuestas, pct), fill=respuestas)) +
  geom_col() +
