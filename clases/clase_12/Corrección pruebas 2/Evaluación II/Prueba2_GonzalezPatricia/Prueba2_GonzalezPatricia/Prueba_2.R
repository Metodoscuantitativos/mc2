...
# En un contraste de hipotesis, la hipotesis nula es una informacion que se somete a prueba estadistica y generalmente representa la situacion de no efecto o 
#no diferencia entre grupos o condiciones. Es la hipotesis que se asume verdadera a menos que haya suficiente evidencia estadistica  en contra de ella.
#Por otro lado, la hipotesis alternativa es la afirmacion opuesta a la hipotesis nula, pues esta representa la situacion que se desea probar, es decir, que hay un
#efecto, una diferencia o una relacion significativa entre variables en el contexto del estudio por lo que en pocas palabras la h. nula refuta con evidencia estadistica
# mientras que la alternativa es la afirmacion que se intenta respaldar si se rechaza la hipotesis nula

### En general se da luces de que ambas hipótesis son opuestas y excluyentes 
### Sin embargo, hacen falta algunas precisiones
...
# RESPUESTA: La diferencia entre la estadística descrptiva y la estadística inferencial es que la descriptiva, describe y resumelos datos de
# manera informativa, utlizando tecnicas como las medidas de tendencia y/o los gráficos, tablas, esta se enfoca en resumir y describuir los datos observados. la estadística inferencial hace inferencias
# o deducciones sobre la muestra en que estamos basando nuestros datos, enfocandoce en hacer generalizaciones sobre poblaciones más amplias
# basandose en datos muestrales.
...
# RESPUESTA: La correlación proporciona una medida mas precisa de la relacion lineal entre dos variables y esta no depende de unidades de medida de las variables, además de proporcionar mayor información quie la covarianza.
# la covarianza mide la dirección de la relación lineal entre dos variables e indica si ambas variables varían juntas y en que dirección. son diferentes al momento de la interpretación de cada una
#pero similares al momento de referirse a lo que son, medidas estadísticas que se utlizan para evaluar la relación de dos variables.
...
str(base)
#RESPUESTA:numero de filas: 147L
#RESPUESTA:numero de columbas: 60L
...
# Puede utilizar el siguiente formato
 ...
 base <- base %>% dplyr::rename("horas_en_rrss" = "rs_01",
                                 "usos_rrss" = "rs_02",
                                 "estres_rss" = "rs_04", 
                                "rrss_masutilizada" = "rs_05")
...
patricia_gonzalez <-  freq(respuestas, prop=TRUE, order = "freq", report.nas = FALSE) %>% 
  tb()
...
ggplot(patricia_gonzalez, aes(x = pct, y = fct_reorder(respuestas, pct), fill=respuestas)) +
  geom_col() +
