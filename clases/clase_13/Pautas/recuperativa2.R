                                    ###         PRUEBA Recuperativa 2        ###

#                                       ***********Instrucciones************


# a. Ir a File y crear un Rproject denominelo: apellido_recuperativa2.Rproj, por ejemplo: "Ocampo_recuperativa2.Rproj" 

# b. Guardar este script (recuperativa_2.R) y base de datos de la encuesta del curso (base_antropologia_limpia.xlsx) en la carpeta en que se encuentra su proyecto apellido_recuperativa2.Rproj
...
# el script se debe denominar igual que el archivo del proyecto, es decir, apellido_recuperativa2.R (sólo cambia su extensión de .Rproject a .R) 
...
# y enviarla a ginowrs@gmail.com y semunoz@uahurtado.cl Con el asunto: apellido recuperativa 2
...
# Seleccione 2 de las siguientes 3 preguntas y responda debajo de la pregunta correspondiente,
# su respuesta debe ser clara y responder específicamente lo que se consulta, (mínimo un párrafo por respuesta, máximo 2). (4 ptos en total) cada pregunta tiene 2 ptos.

# 1. ¿Cuáles son las principales diferencias entre la media aritmética y la mediana como medidas de tendencia central? Proporcione un ejemplo en el que cada medida sea más adecuada.
# 
# 2. ¿Cómo se interpreta el coeficiente de correlación de Pearson? Explique su rango de valores y lo que representan en términos de relación entre dos variables.
# 
# 3. Describa el propósito del análisis de varianza (ANOVA) y cuándo es más apropiado utilizar esta prueba en lugar de una prueba t de Student.
...
#02.02: renombre 5 variables pertenecientes al grupo del cual usted participa(2 ptos)
...
# 03. Suponga que está trabajando para una investigación sobre el descanso y tiempo de ocio
# Su jefe le solicita un gráfico que indique la frecuencia de las actividades de ocio de los estudiantes de antropología.
# Para lo cual debe realizar las siguientes acciones

# 03.1 Cambie el  nombre de la variable TO_04 por actividades_ocio (0.5 ptos) en la misma base de datos

base <- ____ %>% rename(_____=____)


# 03.2 observe las categorías de la variable actividades_ocio, puede utilizar funciones como freq, table, unique, etc. (0.5 ptos)
...
# Para ello separe las respuestas de la variable actividades_ocio por coma (,). 
...
# 03.6 Elimine caracteres invisibles y espacios adicionales(0.5 ptos)
_______ <- gsub("[·]", "", ____) # eliminar el carácter "·"
respuestas <- trimws(respuestas) # eliminar espacios en blanco al inicio y al final

# Normalice respuestas para eliminar diferencias menores (0.5 ptos)
_____ <- tolower(_____) # convierto todo a minúsculas (opcional)
respuestas <- gsub("[[:space:]]+", " ", respuestas) # reemplace múltiples espacios con un solo espacio
